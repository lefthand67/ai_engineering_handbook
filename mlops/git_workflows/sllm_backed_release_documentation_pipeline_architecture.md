...
This handbook defines the **specific technical architecture** we use to generate official project documentation (Changelogs, Release Notes) leveraging the local, resource-constrained **Small Language Models (SLLMs)** (1B to 14B parameters).
...
We implement a **Decoupled Documentation Pipeline**. This is the architectural standard. We explicitly avoid the common anti-pattern of passing massive, raw data (like a complete `git diff/log -p` output) to the SLLM in one monolithic prompt. This anti-pattern causes VRAM saturation, OOM errors, and non-deterministic latency.
...
The following diagram illustrates how the enforced commit standard is processed through the stages to produce the final release artifacts. Note that **Stage 1** is primarily a **Developer** task, but we are aimed to make it **SLLM-assisted** stage also - generatin commit title and body using the small `git diff` file (**REMEMBER**: commits should be atomic and self-contained).
...
flowchart TB
 subgraph MLOps_Doc_Pipeline["**MLOps Documentation Pipeline**"]
    ...
    - *Processor*: **SLLM** (7B–14B) with **system prompt**:
...
- SLLMs only where natural language adaptation is required. 
...
* **Mitigation:** System prompts for SLLMs **MUST** be highly compact, using JSON or constrained Markdown templates instead of verbose prose. All prompts must be **version-controlled alongside model configs** to prevent drift between prompt behavior and model quantization performance. 
...
**Why it Fails**: Combining raw diff analysis and release note generation in one SLLM pass forces the model to juggle low-level syntactic noise and high-level semantic summarization. This leads to:
...
**Empirical evidence**: In SLLM pipelines, decoupling deterministic preprocessing (e.g., `git diff --name-only`, AST extraction) from LLM summarization reduces failure rate by 3–5× and improves cache hit ratio in CI.
...
The choice of the **Decoupled Documentation Pipeline** is an architectural imperative driven by the **resource constraints** of the SLLM stack. This decision ensures high performance, reliability, and low operational cost, directly contrasting the brittle and resource-heavy "Monolithic" anti-pattern.
